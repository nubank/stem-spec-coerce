(ns spec-coerce.spec-alpha2
  (:refer-clojure :exclude [def parse-boolean parse-double parse-long parse-uuid])
  (:require [spec-coerce.spec2-inspec :as spec2-inspec]
...
(defmethod sym->coercer `spec2/or [form] (parse-or form))
(defmethod sym->coercer `spec2/coll-of [form] (parse-coll-of form))
(defmethod sym->coercer `spec2/map-of [form] (parse-map-of form))
(defmethod sym->coercer `spec2/tuple [form] (parse-tuple form))
(defmethod sym->coercer `spec2/multi-spec [form] (parse-multi-spec form))
...
(defmethod sym->coercer `spec2/merge [form]
  (parse-merge form))

(defn nilable-spec? [spec]
  (and (seq? spec)
       (= `spec2/nilable (first spec))))
...
(defn conform
  ...
  ([k x opts]
   (spec2/conform k (coerce k x opts))))
